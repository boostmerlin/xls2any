...
class _YMLConverter(_TargetConverter):
    def __init__(self, parentConverter):
        super(_YMLConverter, self).__init__(parentConverter)

    def get_signature(self):
        desc = "The File is auto generated by xls2any, DO NOT EDIT."
        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        return "# %s\n# TIME: %s\n" % (desc, now)

    def _append(self, text, indent):
        line_code = self.converter.indent * indent + text
    ...
    def generate_array(self, sheet_desc, root):
        ...
        self.converter.append_line(table_var + ":")
        row_cnt = len(root)
        for row in root:
            line_code = self.converter.indent
            if len(row) == 1 or row_cnt != 1:
                line_code += "- "
            if len(row) != 1:
                line_code += "["
            cell_idx = 1
            for cell in row:
                ...
                line_code += cell
                cell_idx += 1
            line_code += _EMPTY if len(row) <= 1 else "]"
            self.converter.append_line(line_code)
        self.converter.append_line("# end of file")
    ...
    def _simple_map(self, node, step, key_name, comment):
        child = node[0]
    ...
    def _gen_tree_code(self, sheet_desc, node, step, key_name, comment):
        ...
        if step >= len(sheet_desc.keys):
            if sheet_desc.simple_map and len(node) == 1:
                self._simple_map(node, step, key_name, comment)
                return
            indent = self.converter.indent * step
            self.converter.append_line(indent + key_name + ": ")
            indent += self.converter.indent
            for kv in node:
                k = kv["k"]
                v = kv["v"]
                line = indent + "%s: %s" % (k, self._format_node(v))
                if type(v) == list and len(line) > 40:
                    self.converter.append_line(indent + "%s: " % k)
                    for vv in v:
                        self.converter.append_line(indent + self.converter.indent + "- %s" % vv)
                else:
                    self.converter.append_line(line)
            return
        ...
        for kv in node:
            ...
            self._gen_tree_code(
                sheet_desc, kv["v"], step + 1, _unquote_key(kv["k"]), comment)
            firstNode = False
...
class _JsonConverter(_TargetConverter):
    ...
    def _format_node(self, node):
        if type(node) == list:
            return self.BEGIN_ARRAY + self.VALUE_SEPARATOR.join(node) + self.END_ARRAY
        else:
            return node
...
class Converter(object):
    ...
    FILE_OVER = 2
    SHEET_OVER = 1
    SUPPORTED_LANGUAGE = {"lua": _LuaConverter, "json": _JsonConverter, "yml": _YMLConverter, "yaml": _YMLConverter}
    VIRTUAL_KEYS = ("VK_INT",)
...
def _parse_argument():
    ...
    parser.add_argument("-t", "--target", dest="target", default="json",
                        help="target language file ext", choices=Converter.SUPPORTED_LANGUAGE.keys())
